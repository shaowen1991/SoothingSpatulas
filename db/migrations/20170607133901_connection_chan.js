...
exports.up = function (knex, Promise) {
  return Promise.all([
    knex.schema.createTableIfNotExists('connections', function (table) {
      ...
      table.integer('users_a_id').references('users.id').onDelete('CASCADE');
      table.integer('users_b_id').references('users.id').onDelete('CASCADE');
      table.string('connection_name', 100).notNullable();
      table.timestamps(true, true);
    }),
    knex.schema.createTableIfNotExists('channels', function (table) {
      table.increments('id').unsigned().primary();
      table.integer('from_id').references('users.id').onDelete('CASCADE');
      table.integer('to_id').references('users.id').onDelete('CASCADE');
